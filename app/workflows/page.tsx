...
import type { Document, Workflow } from "@/lib/models/workflow"
...
function WorkflowsContent() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setWorkflows(await workflowsApi.getWorkflows())
      } catch (error) {
        console.error("Error fetching data:", error)
        toast({
          title: "Error",
          description: "Failed to fetch workflows and documents. Please try again.",
          variant: "destructive",
        })
      } finally {
        setIsLoading(false)
  ...
  const handleUpdateWorkflow = async (updatedWorkflow: Workflow) => {
    try {
      const updated = await workflowsApi.updateWorkflow(updatedWorkflow.id, updatedWorkflow)
      if (updated) {
        const allDocuments = await documentsApi.getDocuments()
        const documentsInWorkflow = allDocuments.filter((doc) => doc.workflowId && doc.status !== "Signed")
        setDocuments(documentsInWorkflow)
